...
from .backbones.senet import (SEBottleneck, SENet, SEResNetBottleneck,
                              SEResNeXtBottleneck)
from .layer import (CenterLoss, CrossEntropyLabelSmooth,
...
class Baseline(nn.Module):
    ...
    def get_optimizer(self, cfg, criterion):
        ...
        if cfg.SOLVER.OPTIMIZER_NAME == "SGD":
            optimizer["model"] = getattr(torch.optim, cfg.SOLVER.OPTIMIZER_NAME)(
    ...
    def get_creterion(self, cfg, num_classes):
        ...
        def criterion_total(score, feat, target):
            loss = (
                criterion["xent"](score, target) + criterion["triplet"](feat, target)[0]
            )
            if cfg.MODEL.CENTER_LOSS == "on":
                loss = loss + cfg.SOLVER.CENTER_LOSS_WEIGHT * criterion["center"](
                    feat, target
                )
            return loss
