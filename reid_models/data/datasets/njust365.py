...
class NJUST365(ReIDSamples):
    ...
    def __init__(self, data_path, combineall=False, season="all", **kwargs):
        ...
        if season in ["spr", "all"]:
            ...
            gallery = self._load_querygallery_images_path(
                gallery_path, idstartfrom=999999
            )
        ...
        if season == "win":
            super(NJUST365, self).__init__(
                win.train, win.query, win.gallery, combineall
            )
        elif season == "spr":
            super(NJUST365, self).__init__(
                spr.train, spr.query, spr.gallery, combineall
            )
        else:
            ...
            gallery = self.combine(
                [copy.deepcopy(win.gallery), copy.deepcopy(spr.gallery)]
            )
            ...
            for folder in folders:
                ...
                for file in files:
                    if ".jpg" in file or ".png" in file:
                        ...
                        txt_file.writelines(
                            "{path},{pid},{cid}\n".format(
                                path=join(join(path, folder), file), pid=pid, cid=cid
                            )
                        )
                        samples.append(
                            [
        ...
        if os.path.exists(txt_path):
            ...
            for file in files:
                if ".jpg" in file:
                    ...
                    txt_file.writelines(
                        "{path},{pid},{cid}\n".format(
                            path=join(path, file), pid=pid, cid=cid
                        )
                    )
                    samples.append(
                        [join(path, file), pid + idstartfrom, cid + idstartfrom]
                    )
            txt_file.close()
        return samples
