...
class Market1501(ReIDSamples):
    ...
    def __init__(self, data_path, combineall=False, download=False):
        ...
        if not os.path.exists(data_path):
            if download:
                ...
                logger.info(
                    "dataset path {} is not existed, start download dataset".format(
                        data_path
                    )
                )
                self.download_dataset(data_path, self.dataset_url)
            else:
                return "dataset path {} is not existed, start download dataset".format(
                    data_path
                )
    ...
    def _analysis_file_name(self, file_name):
        ...
        """
        split_list = (
            file_name.replace(".jpg", "").replace("c", "").replace("s", "_").split("_")
        )
