...
class ReIDSamples:
    ...
    def __init__(self, train, query, gallery, combineall=False, **kwargs):
        if combineall:
            logger.info(
                "[{} Combine All] combine train, query and gallery and training set ... ...".format(
    ...
    def statistics(self, **kwargs):
        ...
        for key, val in kwargs.items():
            if key in ["train", "query", "gallery"]:
                ...
                key_str = str(key)
                if (
                    "combineall" in kwargs.keys()
                    and kwargs["combineall"]
                    and key == "train"
                ):
                    key_str += "(combineall)"
                img_num, pid_num, cid_num = info
                imgs_per_id = (
                    round(img_num / float(pid_num), 2) if img_num is not None else None
                )
                imgs_per_cam = (
                    round(img_num / float(cid_num), 2) if img_num is not None else None
                )
                imgs_per_idcam = (
                    round(img_num / float(pid_num) / float(cid_num), 2)
    ...
    def download_dataset(self, dataset_dir, dataset_url):
        ...
        logger.info('Creating directory "{}"'.format(dataset_dir))
        ...
        logger.info("{} dataset is ready".format(self.__class__.__name__))
