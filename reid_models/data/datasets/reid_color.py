...
class ReIDClothingColors4Market:
    ...
    def __init__(self, attribute_file, upper=True):
        ...
        self.clothing_colors_train = self._select_attributes(
            self.attributes_train, self.clothing_color_names, self.attribute_names_train
        )

    def _select_attributes(
        self, attributes, selected_attribute_names, all_attribute_names
    ):
        """
        ...
        selected_attrs = np.concatenate(
            [np.ones([selected_attrs.shape[0], 1]), selected_attrs], axis=1
        )
...
class ReIDClothingColors4Duke(ReIDClothingColors4Market):
    ...
    def _load_attribute_mat(self, attr_file):
        ...
        for i in range(attr_num):
            for j in range(p_num_train):
                attr_train[i][j] = data[0][0][train][0][0][i][0][j]
        attr_query = np.zeros([attr_num, p_num_test])
        for i in range(attr_num):
            for j in range(p_num_test):
                attr_query[i][j] = data[0][0][query][0][0][i][0][j]
...
if __name__ == "__main__":
    ...
    ReIDClothingColors4Market(f"{Path(__file__).parent/ 'market_attribute.mat'}")
